# The Public Key Cipher (based on the RSA cipher)
#
# Based on chapter 24 of Cracking Codes with Python by Al Sweigart
...
def main():
    ...
    if mode == 'encrypt':
        ...
        pubKeyFilename = 'l-glogov_pubkey.txt'
        ...
        privKeyFilename = 'l-glogov_privkey.txt'
...
def getBlocksFromText(message, blockSize):
    ...
    for char in message:
        if char not in SYMBOLS:
            print('ERROR: The symbol set does not have the character %s' % (char))
            sys.exit()
    blockInts = []
